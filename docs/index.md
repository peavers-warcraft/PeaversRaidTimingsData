...
> `encounterID` is a **DungeonEncounterID** (`DungeonEncounter.db2`), not a Warcraft Logs encounter id. The two are different namespaces; the generator maps between them, so pass the client's id through unchanged.
...
| `report`   | string | Warcraft Logs report code.                                         |
